...
#ifdef INGA_DEBUG
    #pragma message "--- INGA_DEBUG est ACTIF dans allocator.cpp ---"
#else
    #pragma warning "--- ATTENTION : INGA_DEBUG est INACTIF dans allocator.cpp ---"
#endif
...
// 1. SÉCURITÉ
    ...
    {
        INGA_ASSERT(header->canary == 0x494E4741, "Attempt to free invalid or corrupted pointer!");
        // Log d'erreur possible ici : "Tentative de libération d'un pointeur invalide"
        return;
    }

    INGA_ASSERT(header->used == INGA_TRUE, "Double free detected!");
...
void* Allocator::realloc(void* ptr, U64 newSize, U32 align, const char* file, I32 line)
{
  // 1. CAS PARTICULIERS STANDARDS
    ...
    INGA_ASSERT(header->canary == 0x494E4741, "Realloc sur un pointeur invalide !");
